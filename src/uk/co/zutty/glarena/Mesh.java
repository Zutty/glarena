...
public class Mesh {
    ...
    public Mesh(int texture) {
        this.texture = texture;

        numIndeces = 0;
    }

    public static Mesh fromModel(Model model, int texture) {
        Mesh mesh = new Mesh(texture);
        mesh.numIndeces = model.getIndeces().size();

        FloatBuffer vertexBuffer = BufferUtils.createFloatBuffer(model.getVertices().size() * Vertex.STRIDE);
        ShortBuffer indexBuffer = BufferUtils.createShortBuffer(mesh.numIndeces);

        for (Vertex vertex: model.getVertices()) {
            vertex.put(vertexBuffer);
        }

         for(short index: model.getIndeces()) {
        ...
        mesh.vaoId = GL30.glGenVertexArrays();
        ...
        mesh.iboId = GL15.glGenBuffers();
        GL15.glBindBuffer(GL15.GL_ELEMENT_ARRAY_BUFFER, mesh.iboId);
